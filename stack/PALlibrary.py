# ======================================
# PAL ITERATION 23e - CLOTHED EMPEROR CFG TRUTH
# Lifter + CFG + Symbol Resolution
#
# v23e closes the branch-truth custody gap exposed by copy_token@0x1015a1.
# Ordered raw Jcc predicates and HighFunction comparisons are reconciled with
# signed/unsigned relational algebra over two distinct, equal-width,
# non-constant operands.  This proves same/reversed operand orientation and
# direct/complement polarity without turning an unresolved value into False.
#
# v23e also recognizes zero/nonzero tests of proven HighFunction Boolean PHI
# carriers.  This keeps command_arm/command_commit MULTIEQUAL predicates under
# explicit carrier evidence instead of treating a JNZ+MULTIEQUAL pair as an
# unknown direct-flags relation.
#
...
PAL_LIBRARY_VERSION = "PALlibrary_v23e_clothed_emperor_cfg_truth"
...
class PALTerminator:
    ...
    def __init__(self, opcode, inputs, condition=None):
        ...
        self.condition_relation_evidence = None
        self.condition_boolean_carrier_evidence = None

        # Full audit record for POW/diagnostic lookups.
        self.raw_hf_audit = None

        # v23c / FunctionCFG v22 multiway-dispatch evidence.  BRANCHIND
        # carries a computed destination, not necessarily the source selector.
        # CFG records this distinction explicitly and never treats successor
        # ordinal as a case value.
        self.multiway_dispatch = None
        self.multiway_computed_target_sid = None
        self.multiway_selector_sid = None
...
class CFGEdge:
    ...
    def __init__(self, src, dst, etype, **meta):
        ...
        # FunctionCFG v23 / bohdi-emperor canonical edge identity.
        # The tuple is structural only: it never encodes branch truth, PHI
        # reason, successor ordinal, or presentation names.  Those facts remain
        # separate contracts.  FunctionCFG revalidates and freezes this token
        # after all graph repair and IPDOM computation have completed.
        self.edge_id_schema = meta.get("edge_id_schema", "cfg_edge_identity_v1")
        self.edge_src_addr = getattr(src, "addr", None)
        self.edge_dst_addr = getattr(dst, "addr", None)
        self.edge_id = (self.edge_src_addr, self.edge_dst_addr)
        self.edge_key = meta.get("edge_key")
        self.edge_identity_frozen = bool(meta.get("edge_identity_frozen", False))

        # Direct-to-join metadata is deliberately initialized as false.  It is
        # authoritative only after postdominators/IPDOM are available.
        self.direct_to_join = bool(meta.get("direct_to_join", False))
        self.direct_join_addr = meta.get("direct_join_addr")
        self.direct_join_owner_addr = meta.get("direct_join_owner_addr")
        self.direct_join_branch_role = meta.get("direct_join_branch_role")
        self.direct_join_authority = meta.get("direct_join_authority")
        self.direct_join_status = meta.get("direct_join_status", "unclassified")
        ...
        self.condition_relation_evidence = meta.get("condition_relation_evidence")
        self.condition_boolean_carrier_evidence = meta.get(
            "condition_boolean_carrier_evidence"
        )
        ...
        # FunctionCFG v22 multiway edge identity.  These fields are additive
        # and serialization-safe.  Case values are sourced only from
        # HighFunction JumpTable labels, never from CFG successor order.
        self.is_multiway_edge = bool(meta.get("is_multiway_edge", False))
        self.multiway_dispatch_addr = meta.get("multiway_dispatch_addr")
        self.multiway_role = meta.get("multiway_role")
        self.multiway_case_values = list(meta.get("multiway_case_values", []) or [])
        self.multiway_authority = meta.get("multiway_authority")
        self.multiway_status = meta.get("multiway_status")
    ...
    def set_meta(self, **meta):
        ...
        if "condition_relation_evidence" in meta:
            self.condition_relation_evidence = meta[
                "condition_relation_evidence"
            ]
        if "condition_boolean_carrier_evidence" in meta:
            self.condition_boolean_carrier_evidence = meta[
                "condition_boolean_carrier_evidence"
            ]
        ...
        if "is_multiway_edge" in meta:
            self.is_multiway_edge = bool(meta["is_multiway_edge"])
        if "multiway_dispatch_addr" in meta:
            self.multiway_dispatch_addr = meta["multiway_dispatch_addr"]
        if "multiway_role" in meta:
            self.multiway_role = meta["multiway_role"]
        if "multiway_case_values" in meta:
            self.multiway_case_values = list(meta["multiway_case_values"] or [])
        if "multiway_authority" in meta:
            self.multiway_authority = meta["multiway_authority"]
        if "multiway_status" in meta:
            self.multiway_status = meta["multiway_status"]
        if "edge_id_schema" in meta:
            self.edge_id_schema = meta["edge_id_schema"]
        if "edge_key" in meta:
            self.edge_key = meta["edge_key"]
        if "edge_identity_frozen" in meta:
            self.edge_identity_frozen = bool(meta["edge_identity_frozen"])
        if "direct_to_join" in meta:
            self.direct_to_join = bool(meta["direct_to_join"])
        if "direct_join_addr" in meta:
            self.direct_join_addr = meta["direct_join_addr"]
        if "direct_join_owner_addr" in meta:
            self.direct_join_owner_addr = meta["direct_join_owner_addr"]
        if "direct_join_branch_role" in meta:
            self.direct_join_branch_role = meta["direct_join_branch_role"]
        if "direct_join_authority" in meta:
            self.direct_join_authority = meta["direct_join_authority"]
        if "direct_join_status" in meta:
            self.direct_join_status = meta["direct_join_status"]
...
class CFGNode:
    def __init__(self, block):
        ...
        # FunctionCFG v22 additive dispatch contract, populated after IPDOM.
        self.multiway_dispatch = None
...
class PALFunctionObject:
    ...
    def __init__(self, name, entry_addr, addr_range):
        ...
        # v23c FunctionCFG multiway-dispatch handoff.  The lifter retains the
        # live HighFunction long enough for FunctionCFG to consume
        # HighFunction.getJumpTables(); all published records are plain Python
        # values suitable for Icecube serialization.
        self.multiway_dispatch_facts = []
        self.multiway_dispatch_by_block = {}
        self.multiway_dispatch_inventory = {}
        self.multiway_dispatch_events = []

        # v23d bohdi-emperor edge identity / direct-join truth handoff.
        # Published dictionaries use a string edge_key because JSON object keys
        # cannot safely preserve tuple identity.  Every record also carries the
        # canonical two-element edge_id value.
        self.cfg_edge_identity_facts = []
        self.cfg_edge_identity_by_key = {}
        self.cfg_direct_join_edges = []
        self.cfg_direct_join_by_key = {}
        self.cfg_edge_identity_inventory = {}
        self.cfg_edge_identity_events = []
        self.cfg_edge_identity_warnings = []
...
class PALLifter:
    ...
    def _relation_from_raw_jcc_v23e(self, mnemonic):
        """Return the ordered relation tested by a raw Jcc.

        The operator is expressed in raw CMP operand order.  Signed and
        unsigned domains are deliberately distinct; v23e never reconciles
        across them.
        """
        mapping = {
            "JL": ("LT", "signed"),
        ...
        return mapping.get(str(mnemonic or "").upper(), (None, None))

    def _relation_from_hf_opcode_v23e(self, opcode):
        mapping = {
            "INT_SLESS": ("LT", "signed"),
            "INT_SLESSEQUAL": ("LE", "signed"),
            "INT_LESS": ("LT", "unsigned"),
            "INT_LESSEQUAL": ("LE", "unsigned"),
        }
        return mapping.get(str(opcode or "").upper(), (None, None))

    def _relation_swap_v23e(self, relation):
        return {
            "LT": "GT",
            "LE": "GE",
            "GT": "LT",
    ...
    def _relation_complement_v23e(self, relation):
        return {
            "LT": "GE",
            "LE": "GT",
            "GT": "LE",
    ...
    def _var_width_bits_v23e(self, var):
    ...
    def _var_identity_v23e(self, var):
        """Return an audit token; never use a display name as sole identity."""
        if var is None:
            return None
        ...
        if bool(getattr(var, "is_constant", False)):
            return (
                "constant",
                self._var_width_bits_v23e(var),
                self._const_from_var(var),
            )

        high_identity = getattr(var, "high_variable_identity", None)
        if high_identity:
            return ("high", str(high_identity))

        evidence = getattr(var, "numeric_evidence", None)
        if isinstance(evidence, dict):
            high = evidence.get("high_variable") or {}
            identity = high.get("identity_key")
            if identity:
                return ("high", str(identity))
        ...
        if space is not None and offset is not None:
            return ("storage", str(space), offset, size)
        return None

    def _hf_ordered_operand_profile_v23e(self, defop):
        """Prove the narrow operand class on which relational algebra is safe.

        Runtime variable-vs-variable comparisons admit exactly one of four
        identities: same, same-complement, reversed, or reversed-complement.
        Constant thresholds are excluded because a compiler may normalize
        ``x < C`` into ``x <= C-1`` without swapping operands.
        """
        inputs = list(getattr(defop, "inputs", []) or [])
        profile = {
            "status": "unresolved",
            "reason": "ordered_relation_missing_two_operands",
            "operand_count": len(inputs),
            "left_sid": None,
            "right_sid": None,
            "left_identity": None,
            "right_identity": None,
            "left_width_bits": None,
            "right_width_bits": None,
            "left_constant": None,
            "right_constant": None,
        }
        if len(inputs) < 2:
        ...
        left_width = self._var_width_bits_v23e(left)
        right_width = self._var_width_bits_v23e(right)
        left_identity = self._var_identity_v23e(left)
        right_identity = self._var_identity_v23e(right)
        profile.update({
            "left_sid": getattr(left, "ssa_id", None),
            "right_sid": getattr(right, "ssa_id", None),
            "left_identity": left_identity,
            "right_identity": right_identity,
            "left_width_bits": left_width,
            "right_width_bits": right_width,
            "left_constant": left_const,
            "right_constant": right_const,
        })

        if left_const or right_const:
            profile["reason"] = "constant_threshold_requires_exact_value_custody"
            return profile
        if left_width is None or right_width is None:
            profile["reason"] = "ordered_relation_operand_width_unresolved"
            return profile
        if left_width != right_width:
            profile["reason"] = "ordered_relation_operand_width_mismatch"
            return profile
        if left_identity is None or right_identity is None:
            profile["reason"] = "ordered_relation_operand_identity_unresolved"
            return profile
        if left_identity == right_identity:
            profile["reason"] = "ordered_relation_operands_not_distinct"
            return profile

        profile["status"] = "proven_runtime_pair"
        profile["reason"] = (
            "two_distinct_nonconstant_equal_width_hf_operands"
        )
        return profile

    def _ordered_relation_algebra_v23e(self, block):
        """Reconcile raw Jcc and HF ordered predicates as tri-state evidence.

        For a proven runtime operand pair, relation algebra identifies the only
        semantics-preserving combination of operand order and truth polarity.
        Example:

            RAW JL:            LT(A, B)
            HF INT_SLESSEQUAL: LE(B, A)
            identity:          LT(A, B) == not LE(B, A)

        The returned inversion is for the raw taken edge.  The fallthrough edge
        always receives its exact complement.
        """
        term = getattr(block, "terminator", None)
        raw = getattr(block, "raw_terminal_image", None) or {}
        mnemonic = self._mnemonic_upper(raw)
        opcode, defop = self._cond_def_opcode(term)
        raw_relation, raw_domain = self._relation_from_raw_jcc_v23e(mnemonic)
        hf_relation, hf_domain = self._relation_from_hf_opcode_v23e(opcode)
        profile = self._hf_ordered_operand_profile_v23e(defop)

        evidence = {
            "version": "ordered_relation_algebra_v23e",
            "status": "unresolved",
            "mnemonic": mnemonic,
            ...
            "hf_domain": hf_domain,
            "operand_profile": profile,
            "operand_orientation": None,
            "relation_to_hf": None,
            "invert_for_target": None,
            "invert_for_fallthrough": None,
            "reason": None,
        }

        if raw_relation is None or hf_relation is None:
            evidence["reason"] = "ordered_relation_family_not_applicable"
            return evidence
        if raw_domain != hf_domain:
            evidence["reason"] = "ordered_relation_signedness_domain_mismatch"
            return evidence
        if profile.get("status") != "proven_runtime_pair":
            evidence["reason"] = profile.get("reason")
            return evidence

        candidates = [
            ("same", "direct", hf_relation, False),
            (
                "same",
                "complement",
                self._relation_complement_v23e(hf_relation),
                True,
            ),
            (
                "reversed",
                "direct",
                self._relation_swap_v23e(hf_relation),
                False,
            ),
            (
                "reversed",
                "complement",
                self._relation_complement_v23e(
                    self._relation_swap_v23e(hf_relation)
                ),
                True,
            ),
        ...
        matches = [
            item for item in candidates if item[2] == raw_relation
        ]
        if len(matches) != 1:
            evidence["reason"] = (
                "ordered_relation_algebra_not_unique matches=%d" % len(matches)
            )
            return evidence

        orientation, relation_to_hf, _, target_invert = matches[0]
        evidence.update({
            "status": "resolved",
            "operand_orientation": orientation,
            "relation_to_hf": relation_to_hf,
            "invert_for_target": bool(target_invert),
            "invert_for_fallthrough": not bool(target_invert),
            "reason": (
                "raw_%s_%s_equals_%s_hf_%s_%s"
                % (
                    raw_domain,
                    raw_relation,
                    relation_to_hf,
    ...
    def _hf_boolean_lineage_v23e(self, variable, depth=0, seen=None):
        """Prove that a HighFunction condition is a canonical Boolean carrier.

        This deliberately accepts only comparison/Boolean producers and
        transparent wrappers.  MULTIEQUAL is Boolean only when every incoming
        value is independently Boolean; an arbitrary integer PHI is not.
        """
        if seen is None:
            seen = set()
        sid = getattr(variable, "ssa_id", None)
        token = sid if sid is not None else id(variable)
        if token in seen:
            return {
                "status": "unresolved",
                "reason": "boolean_lineage_cycle",
                "root_opcode": None,
                "contains_multiequal": False,
                "depth": depth,
            }
        if depth > 24:
            return {
                "status": "unresolved",
                "reason": "boolean_lineage_depth_limit",
                "root_opcode": None,
                "contains_multiequal": False,
                "depth": depth,
            }
        ...
        if bool(getattr(variable, "is_constant", False)):
            value = self._const_from_var(variable)
            proven = value in (0, 1)
            return {
                "status": "proven_boolean" if proven else "unresolved",
                "reason": (
                    "boolean_constant_0_or_1"
                    if proven else
                    "non_boolean_constant"
                ),
                "root_opcode": "CONSTANT",
                "contains_multiequal": False,
                "depth": depth,
            }

        defop = getattr(variable, "def_op", None)
        opcode = str(getattr(defop, "opcode", "") or "").upper()
        inputs = list(getattr(defop, "inputs", []) or [])
        comparison_ops = {
        ...
        if opcode in comparison_ops:
            return {
                "status": "proven_boolean",
                "reason": "comparison_result_is_boolean",
                "root_opcode": opcode,
                "contains_multiequal": False,
                "depth": depth,
            }
        ...
        transparent = {
            "COPY", "CAST", "INT_ZEXT", "INT_SEXT", "SUBPIECE",
            "BOOL_NEGATE",
        }
        if opcode in transparent and inputs:
            child = self._hf_boolean_lineage_v23e(
                inputs[0], depth=depth + 1, seen=next_seen
            )
            return {
                "status": child.get("status"),
                "reason": "transparent_%s:%s" % (
                    opcode, child.get("reason")
                ),
                "root_opcode": opcode,
                "contains_multiequal": bool(
                    child.get("contains_multiequal", False)
                ),
                "depth": depth,
            }

        boolean_combiners = {"BOOL_AND", "BOOL_OR", "BOOL_XOR"}
        aggregate = opcode == "MULTIEQUAL" or opcode in boolean_combiners
        if aggregate and inputs:
            children = [
                self._hf_boolean_lineage_v23e(
                    child, depth=depth + 1, seen=next_seen
                )
                for child in inputs
            ]
            proven = bool(children) and all(
                child.get("status") == "proven_boolean"
                for child in children
            )
            return {
                "status": "proven_boolean" if proven else "unresolved",
                "reason": (
                    "%s_all_inputs_boolean" % opcode.lower()
                    if proven else
                    "%s_contains_unresolved_input" % opcode.lower()
                ),
                "root_opcode": opcode,
                "contains_multiequal": bool(
                    opcode == "MULTIEQUAL"
                ...
                "input_statuses": [
                    child.get("status") for child in children
                ],
                "depth": depth,
            }
        ...
        return {
            "status": "unresolved",
            "reason": "non_boolean_producer:%s" % (opcode or "NONE"),
            "root_opcode": opcode or None,
            "contains_multiequal": False,
            "depth": depth,
        }
    ...
    def _infer_condition_polarity(self, block):
        ...
        relation_evidence = self._ordered_relation_algebra_v23e(block)
        relation_evidence["legacy_hint_resolved"] = bool(
            hint_target is not None and hint_fall is not None
        )
        relation_evidence["selected_as_authority"] = False
        boolean_evidence = self._hf_boolean_lineage_v23e(
            getattr(term, "condition", None)
        )
        term.condition_relation_evidence = relation_evidence
        term.condition_boolean_carrier_evidence = boolean_evidence
        ...
        phi_boolean_carrier = bool(
            raw_zero_test
            and carrier_branch_family
            and boolean_evidence.get("status") == "proven_boolean"
            and boolean_evidence.get("contains_multiequal") is True
        )

        term.condition_evidence_class = {
            "kind": (
                "materialized_boolean_phi_carrier" if phi_boolean_carrier else
            ...
            "ordered_relation_evidence": dict(relation_evidence or {}),
            "boolean_carrier_evidence": dict(boolean_evidence or {}),
        }
        ...
        # v23e Boolean-PHI carrier rule.  A raw JZ/JNZ immediately consuming a
        # TEST/CMP-zero result is allowed to bind a HighFunction MULTIEQUAL
        # condition only when every PHI input traces to a Boolean producer.
        # This closes command_arm/command_commit without granting arbitrary
        # integer PHIs predicate authority.
        if phi_boolean_carrier:
            if raw_mnemonic in ("JNZ", "JNE"):
                term.condition_truth_authority = (
                    "raw_zero_nonzero_hf_boolean_phi_carrier_v23e"
                )
                return (
                    "target",
                    False,
                    True,
                    base
                    + " class=materialized_boolean_phi"
                    + " binding=raw_nonzero_taken",
                )
            if raw_mnemonic in ("JZ", "JE"):
                term.condition_truth_authority = (
                    "raw_zero_nonzero_hf_boolean_phi_carrier_v23e"
                )
                return (
        ...
        # v23e ordered-relation authority.  This runs after Boolean-carrier
        # exclusion and before the legacy mnemonic/opcode table.  Existing
        # proven v21d mappings retain precedence: without raw operand lineage,
        # relation algebra is an extension for missing table cells, not an
        # authority to rewrite ten-specimen known-good decisions.
        if (
            relation_evidence.get("status") == "resolved"
            and hint_target is None
            and hint_fall is None
        ):
            inv_target = relation_evidence.get("invert_for_target")
            inv_fall = relation_evidence.get("invert_for_fallthrough")
            if isinstance(inv_target, bool) and isinstance(inv_fall, bool):
                relation_evidence["selected_as_authority"] = True
                term.condition_truth_authority = (
                    "raw_jcc_hf_relational_algebra_v23e"
                )
                polarity = "fallthrough" if inv_target else "target"
                return (
                    polarity,
                    inv_target,
                    inv_fall,
    ...
    def _annotate_block_branch_polarity(self, block, block_starts):
        ...
        term.raw_hf_audit = {
            ...
            "condition_relation_evidence": dict(
                getattr(term, "condition_relation_evidence", {}) or {}
            ),
            "condition_boolean_carrier_evidence": dict(
                getattr(term, "condition_boolean_carrier_evidence", {}) or {}
            ),
    ...
    def _attach_pow_image(self):
        self.pal.pow_image = {
            ...
            "cfg_edge_identity_facts": getattr(
                self.pal, "cfg_edge_identity_facts", []
            ),
            "cfg_direct_join_edges": getattr(
                self.pal, "cfg_direct_join_edges", []
            ),
            "cfg_edge_identity_inventory": getattr(
                self.pal, "cfg_edge_identity_inventory", {}
            ),
            "cfg_edge_identity_events": getattr(
                self.pal, "cfg_edge_identity_events", []
            ),
            "cfg_edge_identity_warnings": getattr(
                self.pal, "cfg_edge_identity_warnings", []
            ),
        }
...
class FunctionCFG:
    ...
    def __init__(self, pal_func):
        ...
        self.cfg_version = "FunctionCFG_v23e_clothed_emperor_cfg_truth"
        try:
        ...
        # v22 multiway dispatch truth.  These records are populated only after
        # postdominators/IPDOM exist, so the shared dispatch join can be frozen
        # without SGL guesswork.
        self.multiway_dispatches = []
        self.multiway_dispatch_by_block = {}
        self.multiway_dispatch_events = []
        self.unresolved_multiway_dispatches = []
        self.multiway_dispatch_inventory = {}

        # v23 bohdi-emperor edge identity truth.  The edge token is frozen only
        # after repair, dominators, loop classification, postdominators, and
        # IPDOM are complete.  This prevents downstream layers from rebuilding
        # the same edge with reversed address order or truncated fields.
        self.edge_identity_facts = []
        self.edge_identity_by_key = {}
        self.direct_join_edges = []
        self.direct_join_by_key = {}
        self.edge_identity_events = []
        self.edge_identity_warnings = []
        self.edge_identity_inventory = {}
        ...
        # ------------------------------------------------------------
        # 10. BOHDI EMPEROR EDGE IDENTITY FREEZE
        #     Freeze canonical (src,dst) tokens and classify exact binary
        #     branch edges which go directly to their IPDOM/join.  This stage
        #     records topology only; it does not alter polarity or structure.
        # ------------------------------------------------------------
        self._freeze_edge_identity_truth_v23()
        self._publish_edge_identity_truth_v23()

        # ------------------------------------------------------------
        # 11. MULTIWAY DISPATCH TRUTH
        #     HighFunction JumpTable case labels own value->target mapping.
        #     CFG successor order is never used as a case-value authority.
        # ------------------------------------------------------------
        self._analyze_multiway_dispatches_v22()
        self._publish_multiway_dispatches_v22()

        self.debug_print()


    # =========================================================================
    # v23 BOHDI EMPEROR EDGE IDENTITY / DIRECT-JOIN TRUTH
    # =========================================================================

    @staticmethod
    def _edge_addr_token_v23(value):
        if isinstance(value, int):
            return "0x%x" % value
        if value is None:
            return "NONE"
    ...
    def _edge_key_v23(self, src_addr, dst_addr):
        return "cfg-edge-v1:%s->%s" % (
            self._edge_addr_token_v23(src_addr),
            self._edge_addr_token_v23(dst_addr),
        )

    @staticmethod
    def _binary_branch_role_v23(edge):
        raw_type = getattr(edge, "raw_type", None)
        if raw_type in ("true", "false"):
            return raw_type
        original = getattr(edge, "original_type", None)
    ...
    def _freeze_edge_identity_truth_v23(self):
        """Freeze exact structural edge identity after all CFG repairs.

        The canonical identity is exactly ``(src_addr, dst_addr)``.  It never
        includes PHI reason, SSA names, successor ordinal, or condition truth.
        Direct-to-join classification is granted only to a real edge from a
        binary CBRANCH whose destination is that branch node's immediate
        postdominator.  No edge is created, removed, swapped, or retyped here.
        """
        facts = []
        by_key = {}
        direct = []
        ...
        for src in self._real_nodes_sorted():
            term = getattr(getattr(src, "block", None), "terminator", None)
            opcode = str(getattr(term, "opcode", "") or "").upper()
            real_out = [
                edge for edge in list(getattr(src, "out_edges", []) or [])
                if getattr(getattr(edge, "dst", None), "addr", None) != "EXIT"
            ]
            binary_cbranch = opcode == "CBRANCH" and len(real_out) == 2
            ipdom = getattr(src, "ipdom", None)
            join_addr = getattr(ipdom, "addr", None)
            if join_addr == "EXIT":
                join_addr = None

            for edge in list(getattr(src, "out_edges", []) or []):
                src_addr = getattr(getattr(edge, "src", None), "addr", None)
                dst_addr = getattr(getattr(edge, "dst", None), "addr", None)
                edge_id = (src_addr, dst_addr)
                edge_key = self._edge_key_v23(src_addr, dst_addr)

                prior_id = getattr(edge, "edge_id", None)
                if prior_id not in (None, edge_id):
                    raise RuntimeError(
                        "FunctionCFG v23 edge identity mutation: %r -> %r"
                        % (prior_id, edge_id)
                    )
                if edge_key in by_key:
                    raise RuntimeError(
                        "FunctionCFG v23 duplicate canonical edge identity: %s"
                        % edge_key
                    )

                branch_role = self._binary_branch_role_v23(edge)
                source_is_loop_header = src in self.loop_headers
                is_direct_join = bool(
                    binary_cbranch
                    and join_addr is not None
                    and dst_addr == join_addr
                    and branch_role in ("true", "false")
                )
                direct_join_owner_kind = (
                    "loop_header_exit_edge"
                    if is_direct_join and source_is_loop_header else
                    "binary_branch_arm"
                    if is_direct_join else None
                )
                empty_arm_candidate = bool(
                    is_direct_join and not source_is_loop_header
                )
                status = (
                    "direct_loop_exit_to_ipdom"
                    if is_direct_join and source_is_loop_header else
                    "direct_binary_branch_arm_to_ipdom"
                    if is_direct_join else
                    "binary_branch_non_direct_arm"
                    if binary_cbranch else
                    "not_binary_cbranch_edge"
                )
                authority = (
                    "FunctionCFG_repaired_topology_plus_IPDOM_v23"
                    if is_direct_join else
                    "FunctionCFG_repaired_topology_v23"
                )
                ...
                meta = {
                    ...
                    "direct_to_join": is_direct_join,
                    "direct_join_addr": join_addr if is_direct_join else None,
                    "direct_join_owner_addr": src_addr if is_direct_join else None,
                    "direct_join_branch_role": branch_role if is_direct_join else None,
                    "direct_join_authority": authority if is_direct_join else None,
                    "direct_join_status": status,
                    "direct_join_owner_kind": direct_join_owner_kind,
                    "direct_join_empty_arm_candidate": empty_arm_candidate,
                }
                if hasattr(edge, "set_meta"):
                    edge.set_meta(**meta)
                else:
                    edge.meta = dict(getattr(edge, "meta", {}) or {})
                    edge.meta.update(meta)
                edge.edge_src_addr = src_addr
                edge.edge_dst_addr = dst_addr
                edge.edge_id = edge_id
                edge.direct_join_owner_kind = direct_join_owner_kind
                edge.direct_join_empty_arm_candidate = empty_arm_candidate

                sibling_ids = []
                if binary_cbranch:
                    sibling_ids = [
                        [
                            getattr(getattr(peer, "src", None), "addr", None),
                            getattr(getattr(peer, "dst", None), "addr", None),
                        ]
                        for peer in real_out if peer is not edge
                    ]

                record = {
                    "kind": "cfg_edge_identity_fact_v23",
                    ...
                    "condition_truth_authority": getattr(
                        edge, "condition_truth_authority", None
                    ),
                    "binary_cbranch_owner": binary_cbranch,
                    "branch_owner_addr": src_addr if binary_cbranch else None,
                    "branch_ipdom_addr": join_addr if binary_cbranch else None,
                    "source_is_loop_header": source_is_loop_header,
                    "direct_to_join": is_direct_join,
                    "direct_join_owner_kind": direct_join_owner_kind,
                    "direct_join_empty_arm_candidate": empty_arm_candidate,
                    "direct_join_status": status,
                    "direct_join_authority": authority if is_direct_join else None,
                    "sibling_edge_ids": sibling_ids,
                    "topology_mutated_by_freeze": False,
                }
                facts.append(record)
                ...
                if is_direct_join:
                    ...
                    direct_by_key[edge_key] = record
                    events.append({
                        "kind": "direct_join_edge_frozen_v23",
                        "edge_id": [src_addr, dst_addr],
                        "edge_key": edge_key,
                        "branch_owner_addr": src_addr,
                        "join_addr": dst_addr,
                        "branch_role": branch_role,
                        "owner_kind": direct_join_owner_kind,
                        "empty_arm_candidate": empty_arm_candidate,
                        "authority": authority,
                    })

        facts.sort(key=lambda rec: (
            rec.get("src_addr") is None,
            rec.get("src_addr") if isinstance(rec.get("src_addr"), int) else 0,
            rec.get("dst_addr") is None,
            rec.get("dst_addr") if isinstance(rec.get("dst_addr"), int) else 0,
            str(rec.get("dst_addr")),
        ))
        direct.sort(key=lambda rec: (
            rec.get("src_addr") is None,
            rec.get("src_addr") if isinstance(rec.get("src_addr"), int) else 0,
            rec.get("dst_addr") is None,
            rec.get("dst_addr") if isinstance(rec.get("dst_addr"), int) else 0,
        ...
        self.edge_identity_facts = facts
        self.edge_identity_by_key = by_key
        self.direct_join_edges = direct
        self.direct_join_by_key = direct_by_key
        self.edge_identity_events = events
        self.edge_identity_warnings = warnings
        self.edge_identity_inventory = {
            "kind": "cfg_edge_identity_inventory_v23",
            "version": self.cfg_version,
            "id_schema": "cfg_edge_identity_v1",
            "canonical_identity": "(src_addr,dst_addr)",
            "edges": len(facts),
            "unique_edge_keys": len(by_key),
            "binary_cbranch_edges": sum(
                1 for rec in facts if rec.get("binary_cbranch_owner")
            ),
            "direct_join_edges": len(direct),
            "direct_join_branch_arm_edges": sum(
                1 for rec in direct
                if rec.get("direct_join_owner_kind") == "binary_branch_arm"
            ),
            "direct_join_loop_exit_edges": sum(
                1 for rec in direct
                if rec.get("direct_join_owner_kind") == "loop_header_exit_edge"
            ),
            "direct_join_empty_arm_candidates": sum(
                1 for rec in direct
                if rec.get("direct_join_empty_arm_candidate")
            ),
            "warnings": len(warnings),
            "topology_mutations": 0,
            "polarity_mutations": 0,
            "rule": (
                "freeze_repaired_CFG_edge_identity_then_mark_exact_binary_"
                "CBRANCH_edge_to_IPDOM"
            ),
        }

    def _publish_edge_identity_truth_v23(self):
        facts = list(self.edge_identity_facts or [])
        by_key = dict(self.edge_identity_by_key or {})
        direct = list(self.direct_join_edges or [])
        direct_by_key = dict(self.direct_join_by_key or {})
        inventory = dict(self.edge_identity_inventory or {})
        events = list(self.edge_identity_events or [])
        warnings = list(self.edge_identity_warnings or [])

        self.pal_func.cfg_edge_identity_facts = facts
        self.pal_func.cfg_edge_identity_by_key = by_key
        self.pal_func.cfg_direct_join_edges = direct
        self.pal_func.cfg_direct_join_by_key = direct_by_key
        self.pal_func.cfg_edge_identity_inventory = inventory
        self.pal_func.cfg_edge_identity_events = events
        self.pal_func.cfg_edge_identity_warnings = warnings

        try:
            pow_image = getattr(self.pal_func, "pow_image", None)
            if isinstance(pow_image, dict):
                pow_image["cfg_edge_identity_facts"] = facts
                pow_image["cfg_direct_join_edges"] = direct
                pow_image["cfg_edge_identity_inventory"] = inventory
                pow_image["cfg_edge_identity_events"] = events
                pow_image["cfg_edge_identity_warnings"] = warnings
        except Exception:
            pass

    def edge_identity_for(self, src_or_edge, dst_addr=None):
        if dst_addr is None and hasattr(src_or_edge, "src"):
            src_addr = getattr(getattr(src_or_edge, "src", None), "addr", None)
            dst_addr = getattr(getattr(src_or_edge, "dst", None), "addr", None)
        else:
            src_addr = getattr(src_or_edge, "addr", src_or_edge)
            dst_addr = getattr(dst_addr, "addr", dst_addr)
        return self.edge_identity_by_key.get(
            self._edge_key_v23(src_addr, dst_addr)
        )

    def direct_join_edge_for(self, src_or_edge, dst_addr=None):
        if dst_addr is None and hasattr(src_or_edge, "src"):
            src_addr = getattr(getattr(src_or_edge, "src", None), "addr", None)
            dst_addr = getattr(getattr(src_or_edge, "dst", None), "addr", None)
        else:
            src_addr = getattr(src_or_edge, "addr", src_or_edge)
            dst_addr = getattr(dst_addr, "addr", dst_addr)
        return self.direct_join_by_key.get(
            self._edge_key_v23(src_addr, dst_addr)
        )


    # =========================================================================
    # v22 MULTIWAY DISPATCH TRUTH
    # =========================================================================

    def _safe_list_v22(self, value):
        if value is None:
            return []
        try:
            return list(value)
    ...
    def _address_int_v22(self, value):
        if value is None:
            return None
        if isinstance(value, int):
            return value
        ...
        try:
            ...
            return self._safe_int_v22(value)

    def _real_successors_v22(self, node):
        out = []
        for edge in list(getattr(node, "out_edges", []) or []):
            dst = getattr(edge, "dst", None)
            if dst is None or getattr(dst, "addr", None) == "EXIT":
                continue
            if dst not in out:
                out.append(dst)
        return out

    def _node_containing_address_v22(self, address):
        addr = self._address_int_v22(address)
        if addr is None:
            return None

        exact = self.nodes.get(addr)
        if exact is not None and getattr(exact, "addr", None) != "EXIT":
            return exact

        matches = []
        for node in self._real_nodes():
            block = getattr(node, "block", None)
            start = getattr(block, "hf_start_addr", getattr(node, "addr", None))
            stop = getattr(block, "hf_stop_addr", None)
            if not isinstance(start, int):
                continue
            if isinstance(stop, int) and start <= addr <= stop:
                matches.append(node)
                continue
            term = getattr(block, "terminator", None)
            for candidate in (
                getattr(term, "raw_terminal_addr", None),
                getattr(term, "hf_target_addr", None),
            ):
                if self._address_int_v22(candidate) == addr:
                    matches.append(node)
                    break
    ...
    def _multiway_candidates_v22(self):
        out = []
        for node in self._real_nodes_sorted():
            term = getattr(getattr(node, "block", None), "terminator", None)
            opcode = str(getattr(term, "opcode", "") or "").upper()
            succs = self._real_successors_v22(node)
            if opcode == "BRANCHIND" or len(succs) > 2:
                out.append(node)
        return out

    def _jump_tables_v22(self):
        high = getattr(self.pal_func, "high_function", None)
        if high is None:
            return []
        try:
            return self._safe_list_v22(high.getJumpTables())
        except Exception as exc:
            self.multiway_dispatch_events.append({
                "kind": "multiway_jump_table_read_failed_v22",
                "error": str(exc),
                "authority": "HighFunction.getJumpTables",
            })
            return []

    def _jump_table_switch_node_v22(self, table):
        try:
            switch_addr = self._address_int_v22(table.getSwitchAddress())
        except Exception:
            switch_addr = None

        node = self._node_containing_address_v22(switch_addr)
        if node is not None:
            return node, switch_addr, "switch_address_containment"

        # Fail-closed fallback: only select a candidate if the function has a
        # single unresolved BRANCHIND/multiway node.  No case-value mapping is
        # inferred from this fallback; it merely associates the JumpTable with
        # the only possible dispatcher.
        candidates = self._multiway_candidates_v22()
        if len(candidates) == 1:
            return candidates[0], switch_addr, "unique_multiway_candidate_fallback"
        return None, switch_addr, "dispatcher_unresolved"

    def _trace_dispatch_target_v22(self, variable, max_depth=8):
        """Freeze the computed BRANCHIND target provenance without calling it a selector."""
        root_sid = getattr(variable, "ssa_id", None)
        trace = []
        seen = set()

        def walk(var, depth):
            if var is None or depth > max_depth:
                return
            sid = getattr(var, "ssa_id", None)
            if sid in seen:
                return
            ...
            op = getattr(var, "def_op", None)
            if op is None:
                trace.append({
                    "sid": sid,
                    "kind": "input_or_unresolved_definition",
                    "space": getattr(var, "space", None),
                    "is_constant": bool(getattr(var, "is_constant", False)),
                    "constant_value": (
                        getattr(var, "value", None)
                        if bool(getattr(var, "is_constant", False))
                        else None
                    ),
                })
            ...
            rec = {
                ...
                "input_sids": [getattr(inp, "ssa_id", None) for inp in inputs],
                "input_constants": [
                    getattr(inp, "value", None)
                    if bool(getattr(inp, "is_constant", False))
                    else None
                    for inp in inputs
                ],
            }
            trace.append(rec)
            for inp in inputs:
                if not bool(getattr(inp, "is_constant", False)):
                    walk(inp, depth + 1)

        walk(variable, 0)
        return {
            "computed_target_sid": root_sid,
            "computed_target_name": getattr(variable, "name", None),
            "selector_sid": None,
            "selector_status": "deferred_to_semantic_graph",
            "reason": (
                "BRANCHIND input is a computed destination; FunctionCFG does "
                "not relabel it as the source selector without expression proof"
            ),
            "definition_trace": trace,
        }

    def _jump_table_load_tables_v22(self, table):
        out = []
        try:
            load_tables = self._safe_list_v22(table.getLoadTables())
        except Exception:
            load_tables = []
        for load in load_tables:
            try:
                address = self._address_int_v22(load.getAddress())
            except Exception:
                address = None
            try:
                size = self._safe_int_v22(load.getSize())
            except Exception:
                size = None
            try:
                count = self._safe_int_v22(load.getNum())
            except Exception:
                count = None
            out.append({
                "address": address,
    ...
    def _group_jump_table_cases_v22(self, table, dispatcher):
        issues = []
        try:
            raw_case_addresses = self._safe_list_v22(table.getCases())
        except Exception as exc:
            raw_case_addresses = []
            issues.append("getCases_failed:%s" % exc)
        try:
            raw_labels = self._safe_list_v22(table.getLabelValues())
        except Exception as exc:
            raw_labels = []
            issues.append("getLabelValues_failed:%s" % exc)

        case_addresses = [self._address_int_v22(value) for value in raw_case_addresses]
        labels = [self._safe_int_v22(value) for value in raw_labels]

        if len(case_addresses) != len(labels):
            issues.append(
                "case_label_count_mismatch:%d_cases:%d_labels"
                % (len(case_addresses), len(labels))
            )

        grouped = {}
        unmapped = []
        pair_count = min(len(case_addresses), len(labels))
        dispatcher_successors = set(self._real_successors_v22(dispatcher))

        for index in range(pair_count):
            case_addr = case_addresses[index]
            label = labels[index]
            target = self._node_containing_address_v22(case_addr)

            if label is None:
                unmapped.append({
                    "index": index,
                    "label": None,
                    "case_address": case_addr,
                    "reason": "label_not_integer",
                })
                continue
            if target is None:
                unmapped.append({
                    ...
                    "label": label,
                    "case_address": case_addr,
                    "reason": "case_address_not_mapped_to_cfg_block",
                })
                continue
            if target not in dispatcher_successors:
                unmapped.append({
                    "index": index,
                    "label": label,
                    "case_address": case_addr,
                    "target": getattr(target, "addr", None),
                    "reason": "case_target_not_direct_dispatch_successor",
                })
                continue

            key = getattr(target, "addr", None)
            rec = grouped.setdefault(key, {
                "target": key,
                "values": [],
                "case_addresses": [],
                "source_indices": [],
                "authority": "HighFunction.JumpTable.getCases/getLabelValues",
            })
            if label not in rec["values"]:
                rec["values"].append(label)
            if case_addr not in rec["case_addresses"]:
                rec["case_addresses"].append(case_addr)
            rec["source_indices"].append(index)

        cases = []
        for target_addr in sorted(grouped, key=lambda value: (
            value is None, value if isinstance(value, int) else 0
        )):
            rec = grouped[target_addr]
            rec["values"] = sorted(rec["values"])
            rec["case_addresses"] = sorted(
                value for value in rec["case_addresses"] if isinstance(value, int)
            )
            cases.append(rec)

        if unmapped:
            issues.append("unmapped_case_records:%d" % len(unmapped))

        return {
            "cases": cases,
            "raw_case_addresses": case_addresses,
            "raw_label_values": labels,
            "unmapped": unmapped,
            "issues": issues,
        }

    def _reaches_v22(self, start, target, limit=4096):
        ...
        while work and steps < limit:
            ...
            if node in seen:
                continue
            seen.add(node)
            for succ in self._real_successors_v22(node):
                if succ is target:
                    return True
                if succ not in seen:
                    work.append(succ)
    ...
    def _multiway_join_record_v22(self, dispatcher, arm_nodes):
        join = getattr(dispatcher, "ipdom", None)
        if join is None or getattr(join, "addr", None) == "EXIT":
            return {
                "join": None,
                "status": "unresolved",
                "reason": "dispatcher_ipdom_missing_or_virtual_exit",
                "all_arms_reach_join": False,
            }

        reaches = {
            getattr(arm, "addr", None): self._reaches_v22(arm, join)
            for arm in arm_nodes
        }
        all_reach = bool(reaches) and all(reaches.values())
        return {
            "join": getattr(join, "addr", None),
            "status": "resolved" if all_reach else "candidate_not_reached_by_all_arms",
            "reason": "dispatcher_immediate_postdominator",
            "all_arms_reach_join": all_reach,
            "arm_reachability": reaches,
        }

    def _annotate_multiway_edges_v22(self, dispatcher, record):
        case_by_target = {
            item.get("target"): list(item.get("values", []) or [])
            for item in list(record.get("cases", []) or [])
        }
        default_target = record.get("default_target")
        status = record.get("status")
        authority = record.get("authority")

        for edge in list(getattr(dispatcher, "out_edges", []) or []):
            target_addr = getattr(getattr(edge, "dst", None), "addr", None)
            if target_addr in case_by_target:
                role = "dispatch_case"
                values = case_by_target[target_addr]
            elif target_addr == default_target:
                role = "dispatch_default"
                values = []
            else:
                role = "dispatch_unresolved_successor"
                values = []

            meta = {
                "role": role,
                "is_multiway_edge": True,
                "multiway_dispatch_addr": getattr(dispatcher, "addr", None),
                "multiway_role": role,
                "multiway_case_values": values,
                "multiway_authority": authority,
                "multiway_status": status,
            }
            if hasattr(edge, "set_meta"):
                edge.set_meta(**meta)
            else:
    ...
    def _record_for_jump_table_v22(self, table, dispatcher, switch_addr, association):
        term = getattr(getattr(dispatcher, "block", None), "terminator", None)
        term_inputs = list(getattr(term, "inputs", []) or [])
        computed_target = term_inputs[0] if term_inputs else None

        grouped = self._group_jump_table_cases_v22(table, dispatcher)
        cases = list(grouped.get("cases", []) or [])
        mapped_targets = set(
            item.get("target") for item in cases if item.get("target") is not None
        )
        successors = self._real_successors_v22(dispatcher)
        unmatched = [
            node for node in successors
            if getattr(node, "addr", None) not in mapped_targets
        ]

        default_target = None
        default_status = "unresolved"
        if not grouped.get("unmapped") and len(unmatched) == 1:
            default_target = getattr(unmatched[0], "addr", None)
            default_status = "resolved_by_unique_unlabeled_successor"
        elif not grouped.get("unmapped") and len(unmatched) == 0:
            default_status = "not_exposed_or_all_successors_labeled"
        elif len(unmatched) > 1:
            default_status = "multiple_unlabeled_successors"

        arm_nodes = []
        for node in successors:
            addr = getattr(node, "addr", None)
            if addr in mapped_targets or addr == default_target:
                arm_nodes.append(node)
        join = self._multiway_join_record_v22(dispatcher, arm_nodes)

        issues = list(grouped.get("issues", []) or [])
        if association == "unique_multiway_candidate_fallback":
            issues.append("switch_address_not_mapped_used_unique_candidate")
        if default_status == "multiple_unlabeled_successors":
            issues.append(default_status)
        if join.get("status") != "resolved":
            issues.append("join_%s" % join.get("status"))

        status = "resolved"
        if issues:
            status = "partially_resolved"
        if not cases or grouped.get("unmapped"):
            status = "unresolved"

        record = {
            "kind": "multiway_dispatch_v22",
            "version": "FunctionCFG_v22_multiway_dispatch_truth",
            "dispatcher": getattr(dispatcher, "addr", None),
            "terminator": getattr(term, "opcode", None),
            "switch_address": switch_addr,
            "dispatcher_association": association,
            "successor_count": len(successors),
            "successors": [getattr(node, "addr", None) for node in successors],
            "computed_target": self._trace_dispatch_target_v22(computed_target),
            "cases": cases,
            ...
            "join_evidence": join,
            "load_tables": self._jump_table_load_tables_v22(table),
            "raw_case_addresses": grouped.get("raw_case_addresses", []),
            "raw_label_values": grouped.get("raw_label_values", []),
            "unmapped_case_records": grouped.get("unmapped", []),
            "status": status,
            "issues": issues,
            "authority": "HighFunction.getJumpTables+JumpTable.getCases/getLabelValues",
            "successor_order_used_for_case_values": False,
        }

        try:
            term.multiway_dispatch = dict(record)
            term.multiway_computed_target_sid = (
                record.get("computed_target") or {}
            ).get("computed_target_sid")
            term.multiway_selector_sid = None
        except Exception:
            pass

        try:
            dispatcher.multiway_dispatch = dict(record)
            dispatcher.block.multiway_dispatch = dict(record)
        except Exception:
            pass

        self._annotate_multiway_edges_v22(dispatcher, record)
        return record

    def _unresolved_multiway_record_v22(self, dispatcher, reason):
        term = getattr(getattr(dispatcher, "block", None), "terminator", None)
        term_inputs = list(getattr(term, "inputs", []) or [])
        computed_target = term_inputs[0] if term_inputs else None
        successors = self._real_successors_v22(dispatcher)
        join = self._multiway_join_record_v22(dispatcher, successors)
        record = {
            "kind": "multiway_dispatch_v22",
            "version": "FunctionCFG_v22_multiway_dispatch_truth",
            "dispatcher": getattr(dispatcher, "addr", None),
            "terminator": getattr(term, "opcode", None),
            "switch_address": None,
            "dispatcher_association": "cfg_candidate_only",
            "successor_count": len(successors),
            "successors": [getattr(node, "addr", None) for node in successors],
            "computed_target": self._trace_dispatch_target_v22(computed_target),
            "cases": [],
            ...
            "raw_case_addresses": [],
            "raw_label_values": [],
            "unmapped_case_records": [],
            "status": "unresolved",
            "issues": [reason],
            "authority": "CFG_topology_only_no_case_value_authority",
            "successor_order_used_for_case_values": False,
        }
        try:
            dispatcher.multiway_dispatch = dict(record)
    ...
    def _analyze_multiway_dispatches_v22(self):
        candidates = self._multiway_candidates_v22()
        tables = self._jump_tables_v22()
        records = []
        claimed = set()

        for table in tables:
            dispatcher, switch_addr, association = self._jump_table_switch_node_v22(table)
            if dispatcher is None:
                self.multiway_dispatch_events.append({
                    "kind": "multiway_jump_table_dispatcher_unresolved_v22",
                    "switch_address": switch_addr,
                    "authority": "HighFunction.getJumpTables",
                })
                continue
            if dispatcher in claimed:
                self.multiway_dispatch_events.append({
                    "kind": "multiway_duplicate_jump_table_for_dispatcher_v22",
                    "dispatcher": getattr(dispatcher, "addr", None),
                    "switch_address": switch_addr,
                })
                continue
            claimed.add(dispatcher)
            record = self._record_for_jump_table_v22(
                table, dispatcher, switch_addr, association
            )
            records.append(record)
            self.multiway_dispatch_events.append({
                "kind": "multiway_dispatch_resolved_v22",
                "dispatcher": record.get("dispatcher"),
                "status": record.get("status"),
                "case_targets": len(record.get("cases", []) or []),
                "default_target": record.get("default_target"),
        ...
        for dispatcher in candidates:
            if dispatcher in claimed:
                continue
            record = self._unresolved_multiway_record_v22(
                dispatcher,
                "no_matching_HighFunction_JumpTable",
            )
            records.append(record)
            self.multiway_dispatch_events.append({
                "kind": "multiway_dispatch_unresolved_v22",
                "dispatcher": record.get("dispatcher"),
                "successors": list(record.get("successors", []) or []),
                "reason": "no_matching_HighFunction_JumpTable",
            })
        ...
        self.multiway_dispatches = records
        self.multiway_dispatch_by_block = {
        ...
        self.unresolved_multiway_dispatches = [
            rec for rec in records if rec.get("status") != "resolved"
        ]
        self.multiway_dispatch_inventory = {
            "kind": "multiway_dispatch_inventory_v22",
            "version": self.cfg_version,
            "dispatchers": len(records),
            "resolved": sum(1 for rec in records if rec.get("status") == "resolved"),
            "partially_resolved": sum(
                1 for rec in records if rec.get("status") == "partially_resolved"
            ),
            "unresolved": sum(
                1 for rec in records if rec.get("status") == "unresolved"
            ),
            "jump_tables": len(tables),
            "case_value_authority": "HighFunction.JumpTable.getLabelValues",
            "case_target_authority": "HighFunction.JumpTable.getCases",
            "successor_order_used_for_case_values": False,
            "rule": (
                "parallel_Ghidra_jump_table_labels_and_addresses_only;"
                "unlabeled_unique_CFG_successor_may_be_default"
            ),
        }

    def _publish_multiway_dispatches_v22(self):
        records = list(self.multiway_dispatches or [])
        by_block = dict(self.multiway_dispatch_by_block or {})
        inventory = dict(self.multiway_dispatch_inventory or {})
        events = list(self.multiway_dispatch_events or [])

        self.pal_func.multiway_dispatch_facts = records
        self.pal_func.multiway_dispatch_by_block = by_block
        self.pal_func.multiway_dispatch_inventory = inventory
        self.pal_func.multiway_dispatch_events = events
        self.pal_func.cfg_multiway_dispatches = records
        self.pal_func.cfg_multiway_dispatch_by_block = by_block

        try:
            pow_image = getattr(self.pal_func, "pow_image", None)
            if isinstance(pow_image, dict):
                pow_image["multiway_dispatch_facts"] = records
                pow_image["multiway_dispatch_inventory"] = inventory
                pow_image["multiway_dispatch_events"] = events
        except Exception:
            pass

    def is_multiway_dispatch(self, node_or_addr):
        return self.multiway_dispatch_for(node_or_addr) is not None

    def multiway_dispatch_for(self, node_or_addr):
        if hasattr(node_or_addr, "addr"):
            addr = getattr(node_or_addr, "addr", None)
        else:
            addr = node_or_addr
        return self.multiway_dispatch_by_block.get(addr)
    ...
    def _build_raw_edges(self, pal_func):
        ...
        for b in getattr(pal_func, "blocks", []):
            ...
            # Multiway computed branch.  Keep every successor, but leave
            # value ownership unresolved until HighFunction JumpTable evidence
            # is consumed after IPDOM construction.
            if term is not None and getattr(term, "opcode", None) == "BRANCHIND":
                for s in succs:
                    ...
                    if dst is not None:
                        self._add_edge(
                            ...
                            dst,
                            "uncond",
                            role="multiway_successor_unclassified_v22",
                            is_multiway_edge=True,
                            multiway_dispatch_addr=getattr(node, "addr", None),
                            multiway_role="unclassified_successor",
                            multiway_status="pending_jump_table_evidence",
                            multiway_authority="HighFunction_successor_topology_only",
                        )
                continue
    ...
    def debug_print(self):
        ...
        for n in self._real_nodes_sorted():
            for e in getattr(n, "out_edges", []):
                ...
                edge_key = getattr(e, "edge_key", None)
                direct_join = bool(getattr(e, "direct_to_join", False))
                print(
                    "%s -> %s  %s raw=%s role=%s pol=%s inv=%s "
                    "edge=%s direct_join=%s" % (
                        s_txt, d_txt, e.type, raw, role, pol, inv,
                        edge_key, direct_join,
                    )
                )
        ...
        if self.multiway_dispatches:
            print("\n[MULTIWAY DISPATCH v22]")
            print(self.multiway_dispatch_inventory)
            for rec in self.multiway_dispatches:
                print({
                    ...
                    "cases": [
                        ...
                        for item in list(rec.get("cases", []) or [])
                    ],
                    "default_target": rec.get("default_target"),
                    "default_status": rec.get("default_status"),
                    "join": rec.get("join"),
        ...
        if self.edge_identity_facts:
            print("\n[EDGE IDENTITY v23 / BOHDI EMPEROR]")
            print(self.edge_identity_inventory)
            for rec in self.direct_join_edges:
                print({
                    "edge_id": rec.get("edge_id"),
                    "edge_key": rec.get("edge_key"),
                    "branch_owner_addr": rec.get("branch_owner_addr"),
                    "join_addr": rec.get("dst_addr"),
                    "branch_role": rec.get("branch_role"),
                    "authority": rec.get("direct_join_authority"),
                })
